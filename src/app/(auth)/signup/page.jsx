...
export default function SignupPage() {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const res = await fetch(
          `/api/user/check-username?u=${encodeURIComponent(user.username)}`
        );
  ...
  const canSubmit =
    user.email && user.name && usernameValid && available && passwordValid && !submitting;
          ...
          <form onSubmit={userSignUp} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-white/50">
                  Min 8 chars, mix upper/lower/digit/symbol
                </span>
              </div>
              <input
                id="password"
              ...
              <div className="mt-2 h-2 w-full rounded-full bg-white/10">
                <div
                  className={`h-2 rounded-full transition-all ${strengthBarClasses}`}
                />
              </div>
